...
DrawTriangleCommon* DrawTriangleCommon::instance = nullptr;

// シングルトンのインスタンスを取得
DrawTriangleCommon* DrawTriangleCommon::GetInstance() {
	if (instance == nullptr)
	{
		instance = new DrawTriangleCommon;
	}
	return instance;
}

void DrawTriangleCommon::Finalize()
{
	if (instance) {
		delete instance;
		instance = nullptr;
...
void DrawTriangleCommon::Initialize()
{
	// 引数で受け取ってメンバ変数に記録する
...
void DrawTriangleCommon::CommonSetting()
{
	//ルートシグネチャをセットするコマンド
...
void DrawTriangleCommon::RootSignature()
{
...
void DrawTriangleCommon::GraphicsPipelineState()
{
	// ルートシグネチャの生成
	RootSignature();
	...
	vertexShaderBlob_ = dxCommon_->CompileShader(L"Resources/shaders/DrawTriangle.VS.hlsl",
	...
	pixelShaderBlob_ = dxCommon_->CompileShader(L"Resources/shaders/DrawTriangle.PS.hlsl",
...
void DrawTriangleCommon::InputLayout()
{
	// POSITION の設定
...
void DrawTriangleCommon::BlendState()
{
	// すべての色要素を書き込む
...
void DrawTriangleCommon::RasterizerState()
{
	// 裏面(時計回り)を表示しない
...
void DrawTriangleCommon::DepthStencilState()
{
